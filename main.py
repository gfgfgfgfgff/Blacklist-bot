...
# ✅ CORRECTION : Liste des IDs admin au lieu d'un seul
ADMIN_USER_IDS = [1399234120214909010, 1425947830463365120]
...
def remove_from_blacklist(uid):
    db_cursor.execute('DELETE FROM blacklist WHERE user_id=?', (uid,))
    db_conn.commit()
...
def get_blacklist_user(uid):
    db_cursor.execute('SELECT * FROM blacklist WHERE user_id=?', (uid,))
...
def remove_from_whitelist(uid):
    db_cursor.execute('DELETE FROM whitelist WHERE user_id=?', (uid,))
    db_conn.commit()
    return db_cursor.rowcount > 0

def is_in_whitelist(uid):
    db_cursor.execute('SELECT 1 FROM whitelist WHERE user_id=?', (uid,))
...
def set_log_channel(gid, ltype, cid):
    db_cursor.execute('INSERT OR REPLACE INTO logs_config VALUES(?,?,?)', (gid, ltype, cid))
    db_conn.commit()
...
def get_all_logs(gid):
    db_cursor.execute('SELECT log_type, channel_id FROM logs_config WHERE guild_id=?', (gid,))
    return db_cursor.fetchall()

def update_bl_limit(uid, count, last_reset):
    db_cursor.execute('INSERT OR REPLACE INTO bl_limits VALUES(?,?,?)', (uid, count, last_reset))
    db_conn.commit()

def get_bl_limit(uid):
    db_cursor.execute('SELECT count, last_reset FROM bl_limits WHERE user_id=?', (uid,))
    return db_cursor.fetchone()
...
def get_user_grade(uid, gid):
    # ✅ CORRECTION : Vérifier si l'utilisateur est dans la liste des admins
...
def remove_user_grade(uid, gid):
    db_cursor.execute('DELETE FROM user_grades WHERE user_id=? AND guild_id=?', (uid, gid))
    db_conn.commit()
    return db_cursor.rowcount > 0

def get_all_users_with_grade(gid, grade):
    db_cursor.execute('SELECT user_id FROM user_grades WHERE guild_id=? AND grade=?', (gid, grade))
    return db_cursor.fetchall()
...
def remove_protect(uid):
    db_cursor.execute('DELETE FROM protect WHERE user_id=?', (uid,))
    db_conn.commit()
    return db_cursor.rowcount > 0

def is_protected(uid):
    db_cursor.execute('SELECT 1 FROM protect WHERE user_id=?', (uid,))
    return db_cursor.fetchone() is not None

def get_protect_list():
    db_cursor.execute('SELECT * FROM protect ORDER BY timestamp DESC')
    return db_cursor.fetchall()

def clear_protect():
    db_cursor.execute('DELETE FROM protect')
    db_conn.commit()
    return db_cursor.rowcount
...
class PaginatorWithCounter(discord.ui.View):
    ...
    async def page_counter(self, i, b):
        pass
...
def has_required_grade(min_grade=None):
    async def p(ctx):
        ...
        if not min_grade:
            ...
            if g and GRADES.get(g, 0) >= GRADES.get(min_grade, 0):
                return True
        await ctx.send(embed=discord.Embed(description="Tu na pas la permission d'utiliser cette commande", color=0xFFFFFF))
        return False
...
def create_log_embed(title, fields):
    ...
    e.set_thumbnail(url=LOG_THUMBNAIL)
    e.set_footer(text=get_current_time_french())
    return e
...
def time_ago(ts):
    try:
        ...
        if d.days > 0:
            return "Il y a 1 jour" if d.days == 1 else f"Il y a {d.days} jours"
        if d.seconds >= 3600:
            h = d.seconds // 3600
            return "Il y a 1 heure" if h == 1 else f"Il y a {h} heures"
        if d.seconds >= 60:
            m = d.seconds // 60
            return "Il y a 1 minute" if m == 1 else f"Il y a {m} minutes"
        return "À l'instant"
    except:
...
def check_bl_limit(uid, grade):
    # ✅ CORRECTION : Vérifier si l'utilisateur est admin ou whitelist
    if int(uid) in ADMIN_USER_IDS or is_in_whitelist(str(uid)):
        return True, ""
    r = get_bl_limit(str(uid))
    if not r:
        update_bl_limit(str(uid), 0, datetime.now().isoformat())
        return True, ""
    c, last = r
    last = datetime.fromisoformat(last)
    if datetime.now() - last > timedelta(seconds=BL_COOLDOWN):
        update_bl_limit(str(uid), 0, datetime.now().isoformat())
        return True, ""
...
def increment_bl_count(uid):
    ...
    r = get_bl_limit(str(uid))
    if not r:
        update_bl_limit(str(uid), 1, datetime.now().isoformat())
    else:
        c, last = r
        update_bl_limit(str(uid), c + 1, last)
...
async def send_log(ctx, ltype, fields):
    ...
    try:
        await ch.send(embed=create_log_embed(title, fields))
    except:
        pass
...
async def cooldown_global(ctx):
    ...
    if grade != "Créateur++" and ctx.author.id not in ADMIN_USER_IDS:
...
async def help(ctx):
    ...
    e1.set_footer(text=f"Page 1/4 • {get_current_time_french()}")
...
async def grades(ctx):
    l = []
    for g, v in sorted(GRADES.items(), key=lambda x: x[1], reverse=True):
        l.append("──────────────")
        l.append(f"{g} • Permission {v}")
    l.append("──────────────")
...
async def handle_grade_command(ctx, mid, gname, gdisplay):
    if not mid:
        ...
        if not users:
            return await ctx.send(embed=create_white_embed(f"**Liste des {gdisplay}**\n\nAucun utilisateur n'a le grade {gdisplay}."))
        ml = []
        for uid in users:
            uid = uid[0]
            try:
                u = await bot.fetch_user(uid)
                ml.append(f"{u.mention}\n`{u.id}`")
            except:
                ml.append(f"<@{uid}>\n`{uid}`")
    ...
    if ctx.author.id not in ADMIN_USER_IDS:
        if not eg:
            return await ctx.send(embed=create_white_embed("Tu na pas la permission d'utiliser cette commande"))
        if gname == "Créateur++" and eg != "Créateur++":
            return await ctx.send(embed=create_white_embed("Tu na pas la permission d'utiliser cette commande"))
        if gname == "Créateur" and eg not in ["Créateur++", "Créateur"]:
            return await ctx.send(embed=create_white_embed("Tu na pas la permission d'utiliser cette commande"))
        if gname in ["Sys+", "Sys", "Owner"] and eg == "Créateur" and not is_in_whitelist(str(ctx.author.id)):
            return await ctx.send(embed=create_white_embed("Tu na pas la permission d'utiliser cette commande"))
        if GRADES[gname] >= GRADES[eg]:
            return await ctx.send(embed=create_white_embed("Tu ne peux pas donner un grade égal ou supérieur au tien"))
    ...
    try:
        ...
        grade_author = 'Créateur++' if ctx.author.id in ADMIN_USER_IDS else eg
        await send_log(ctx, "rank", {"Donné par": f"{ctx.author.mention} ({grade_author})", "À": m.mention, "Grade donné": gdisplay})
...
async def owner(ctx, member=None):
    await handle_grade_command(ctx, member, "Owner", "Owner")


@bot.command()
@has_required_grade("Créateur")
async def sys(ctx, member=None):
    await handle_grade_command(ctx, member, "Sys", "Sys")


@bot.command()
@has_required_grade("Créateur")
async def sysplus(ctx, member=None):
    await handle_grade_command(ctx, member, "Sys+", "Sys+")


@bot.command(name="sys+")
@has_required_grade("Créateur")
async def sys_plus(ctx, member=None):
    await handle_grade_command(ctx, member, "Sys+", "Sys+")


@bot.command()
@has_required_grade("Créateur++")
async def crea(ctx, member=None):
    await handle_grade_command(ctx, member, "Créateur", "Créateur")


@bot.command()
@has_required_grade("Créateur++")
async def creapp(ctx, member=None):
    await handle_grade_command(ctx, member, "Créateur++", "Créateur++")


@bot.command(name="crea++")
@has_required_grade("Créateur++")
async def crea_pp(ctx, member=None):
    await handle_grade_command(ctx, member, "Créateur++", "Créateur++")
...
async def ungrade(ctx, member=None):
    if not member:
        return await ctx.send(embed=create_white_embed("Usage : `&ungrade @user/id`"))
    r = await get_user_by_id_or_mention(ctx, member)
    if not r or not r[0]:
        return await ctx.send(embed=create_white_embed("❌ Utilisateur introuvable."))
    m, ison = r
    cg = get_user_grade(m.id, ctx.guild.id)
    if not cg:
        return await ctx.send(embed=create_white_embed(f"{m.mention} n'a aucun grade."))
    eg = get_user_grade(ctx.author.id, ctx.guild.id)
    # ✅ CORRECTION : Vérifier si l'utilisateur est admin
    if ctx.author.id not in ADMIN_USER_IDS:
        if not eg:
            return await ctx.send(embed=create_white_embed("Tu na pas la permission d'utiliser cette commande"))
        if cg == "Créateur++":
            return await ctx.send(embed=create_white_embed("Tu ne peux pas retirer le grade d'un Créateur++"))
        if GRADES[cg] >= GRADES[eg]:
            return await ctx.send(embed=create_white_embed("Tu ne peux pas retirer un grade égal ou supérieur au tien"))
    remove_user_grade(m.id, ctx.guild.id)
    await ctx.send(embed=create_white_embed(f"{m.mention} n'est plus (**{cg}**)"))
    grade_author = 'Créateur++' if ctx.author.id in ADMIN_USER_IDS else eg
    await send_log(ctx, "unrank", {"Retiré par": f"{ctx.author.mention} ({grade_author})", "De": m.mention, "Grade retiré": cg})
...
async def bl(ctx, identifier=None, *, reason=None):
    if ctx.message.reference and ctx.message.reference.message_id and not identifier:
        try:
            identifier = str((await ctx.channel.fetch_message(ctx.message.reference.message_id)).author.id)
        except:
            pass
    if not identifier:
    ...
    if not r or not r[0]:
        return await ctx.send(embed=create_white_embed("❌ Utilisateur introuvable."))
    tm, ison = r
    if tm.id == ctx.author.id:
        return await ctx.send(embed=create_white_embed("Wsh ? T'es con ou quoi? Tu veux te suicider?"))

    if is_protected(tm.id):
        db_cursor.execute('SELECT added_by FROM protect WHERE user_id=?', (tm.id,))
    ...
    if get_blacklist_user(tm.id):
        return await ctx.send(embed=create_white_embed("Cet utilisateur est déjà dans la blacklist."))
    eg = get_user_grade(ctx.author.id, ctx.guild.id)
    # ✅ CORRECTION : Vérifier si l'utilisateur est admin
    if ctx.author.id not in ADMIN_USER_IDS and not eg:
        return await ctx.send(embed=create_white_embed("Tu na pas la permission d'utiliser cette commande"))
    if ison and isinstance(tm, discord.Member) and tm.top_role >= ctx.author.top_role and ctx.author.id not in ADMIN_USER_IDS:
        return await ctx.send(embed=create_white_embed(f"Tu ne peux pas blacklist {tm.mention} car il est égal ou supérieur a toi"))
    if reason and ctx.author.id not in ADMIN_USER_IDS:
        ...
        if eg2 not in ["Créateur", "Créateur++"]:
            for w in get_blocked_words():
                if w in reason.lower():
                    return await ctx.send(embed=create_white_embed("Merci de mettre une raison valable"))
    ...
    if ison and isinstance(tm, discord.Member):
        ...
        if tg == "Créateur++":
            return await ctx.send(embed=create_white_embed("Impossible de blacklist un Créateur++."))
        if ctx.author.id not in ADMIN_USER_IDS and tg and GRADES[eg] <= GRADES[tg]:
            return await ctx.send(embed=create_white_embed(f"Tu ne peux pas blacklist {tm.mention} car il est égal ou supérieur a toi"))
        td = tg or "Aucun grade"
    if ctx.author.id not in ADMIN_USER_IDS and not is_in_whitelist(str(ctx.author.id)):
        c, msg = check_bl_limit(str(ctx.author.id), eg)
        if not c:
            return await ctx.send(embed=create_white_embed(msg))
    ban = False
    if ison:
        try:
            await tm.ban(reason=f"Blacklist par {ctx.author}: {reason}")
            ban = True
        except:
            ban = False
...
async def unbl(ctx, identifier=None):
    if ctx.message.reference and ctx.message.reference.message_id and not identifier:
        try:
            ...
            pass
    if not identifier:
        return await ctx.send(embed=create_white_embed("MAUVAISE UTILISATION\nUsage : `&unbl id/@`"))
    r = await get_user_by_id_or_mention(ctx, identifier)
    if not r or not r[0]:
        return await ctx.send(embed=create_white_embed("❌ Utilisateur introuvable."))
    m, ison = r
    ex = get_blacklist_user(m.id)
    if not ex:
        return await ctx.send(embed=create_white_embed("Cet utilisateur n'est pas dans la blacklist."))
    uid, uname, grade, reason, added_by, added_by_name, banned, on_server, ts = ex
    eg = get_user_grade(ctx.author.id, ctx.guild.id)
    # ✅ CORRECTION : Vérifier si l'utilisateur est admin
    if ctx.author.id not in ADMIN_USER_IDS and not eg:
        return await ctx.send(embed=create_white_embed("Tu na pas la permission d'utiliser cette commande"))
    if added_by != ctx.author.id and ctx.author.id not in ADMIN_USER_IDS:
        try:
            bg = get_user_grade(added_by, ctx.guild.id)
            if bg and GRADES[bg] > GRADES[eg]:
                return await ctx.send(embed=create_white_embed(f"Tu ne peux pas unbl cette utilisateur car il a etait bl par un **({bg})**"))
        except:
            pass
    if added_by in ADMIN_USER_IDS and ctx.author.id not in ADMIN_USER_IDS:
        try:
            return await ctx.send(embed=create_white_embed(f"Cette utilisateur a etait Bl par <@{added_by}>"))
        except:
            return await ctx.send(embed=create_white_embed(f"Cette utilisateur a etait Bl par @akusa"))
    if ison:
        try:
            try:
                await ctx.guild.unban((await ctx.guild.fetch_ban(discord.Object(id=m.id))).user, reason=f"Unblacklist par {ctx.author}")
            except:
                pass
        except:
            pass
    try:
        await m.send(f"Vous avez été unbl de `Akusa` #🎐\n\nVoici le lien du serveur : https://discord.gg/fH2ur9ffSa")
    except:
        pass
...
async def unbanall(ctx):
    # ✅ CORRECTION : Vérifier si l'utilisateur est admin
    if ctx.author.id not in ADMIN_USER_IDS and get_user_grade(ctx.author.id, ctx.guild.id) != "Créateur++":
    ...
    try:
        async for b in ctx.guild.bans():
            try:
                await ctx.guild.unban(b.user, reason=f"Unbanall par {ctx.author}")
                c += 1
            except:
                pass
    except:
...
async def bllist(ctx):
    data = get_blacklist()
    if not data:
        return await ctx.send(embed=create_white_embed("Aucun utilisateur blacklist"))
    p = []
    for i in range(0, len(data), 10):
        ...
        e.set_footer(text=f"blacklist : {len(data)}")
        p.append(e)
    if len(p) == 1:
        await ctx.send(embed=p[0])
    else:
        await ctx.send(embed=p[0], view=PaginatorWithCounter(p, len(data)))
...
async def blinfo(ctx, identifier):
    ...
    if ab:
        l.append(f"Blacklister par : <@{ab}>")
        l.append(f"`{ab}`")
    else:
        l.append("Blacklister par : Inconnu")
    l.append("")
    l.append(f"raison: `{r}`")
    l.append("")
    l.append(f"Il y'a {time_ago(ts).replace('Il y a ', '')}")
    e = create_white_embed("\n".join(l))
    if hasattr(m, 'avatar') and m.avatar:
        e.set_thumbnail(url=m.avatar.url)
...
async def grade(ctx, identifier=None):
    ...
    if ison and isinstance(tm, discord.Member):
        ...
        await ctx.send(embed=create_white_embed(f"{tm.mention} n'est pas sur le serveur, impossible de vérifier son grade"))
...
async def unwl(ctx, identifier=None):
    ...
    if remove_from_whitelist(m.id):
        ...
        await ctx.send(embed=create_white_embed(f"{m.mention} n'est pas dans la whitelist." if ison else f"L'utilisateur `{m.name}` (ID: {m.id}) n'est pas dans la whitelist."))
...
async def clearwl(ctx):
    ...
    if c > 0:
        await send_log(ctx, "clearwl", {"Vidée par": ctx.author.mention, "Nombre retiré": str(c)})
...
async def wllist(ctx):
    ...
    if data:
        ...
        l.append("Aucun utilisateur dans la whitelist")
    await ctx.send(embed=create_white_embed("\n".join(l)))
...
async def logs(ctx):
    ...
    if not data:
        return await ctx.send(embed=create_white_embed("Aucun salon de logs configuré."))
    l = ["Logs\n"]
...
async def changelimit(ctx, grade: str, limit: int):
    ...
    if limit < 0 or limit > 9999:
        return await ctx.send(embed=create_white_embed("Limite invalide. Utilise un nombre entre 0 et 9999."))
...
async def savedb(ctx):
    try:
        ...
        await ctx.send(embed=create_white_embed("❌ Erreur envoi DM"))
...
async def setdb(ctx):
    ...
    try:
        ...
        await ctx.send(embed=create_white_embed("❌ Erreur restauration"))
...
async def block(ctx, action=None, *, word=None):
    if not action:
        return await ctx.send(embed=create_white_embed("Usage : `&block add/remove/list/clear [mot]`"))
    a = action.lower()
    if a == "add":
        if not word:
            return await ctx.send(embed=create_white_embed("Usage : `&block add mot`"))
        if is_word_blocked(word):
            return await ctx.send(embed=create_white_embed(f"Le mot `{word}` est déjà bloqué."))
        ...
        if not word:
            return await ctx.send(embed=create_white_embed("Usage : `&block remove mot`"))
        ...
        w = get_blocked_words()
        if not w:
            await ctx.send(embed=create_white_embed("Aucun mot bloqué."))
        else:
            await ctx.send(embed=create_white_embed("**Liste des mots bloqués :**\n" + "\n".join(f"• `{x}`" for x in w)))
    elif a == "clear":
        c = clear_blocked_words()
        await ctx.send(embed=create_white_embed(f"{c} mot(s) ont été supprimés de la liste."))
    else:
        await ctx.send(embed=create_white_embed("Action invalide. Utilise `add`, `remove`, `list` ou `clear`."))
...
async def protect(ctx, *, id=None):
    if not id:
        return await ctx.send(embed=create_white_embed("Usage : `&protect @user/id`"))
...
async def protectlist(ctx):
    d = get_protect_list()
    if not d:
        return await ctx.send(embed=create_white_embed("Aucun utilisateur protégé."))
